...
const loginUser = async (req, res) => {
  ...
  if (findUser) {
    ...
    if (comparePasswords) {
      const jwt = token.sign({ Email }, process.env.JWT_KEY, {
        expiresIn: "1d",
      });
      ...
      return res.send({
        message:
          "You have logged in successfully now you can utilize the jwt token",
  ...
  return res.send({
    message: "user with thise email id doesnt exist , kindly  create a account",
  });
};
  ...
  try {
    cloudinary.uploader
      .upload_stream({ resource_type: "auto" }, async (error, result) => {
        if (error) {
          return res.status(500).json("couldnt upload a image");
        }

        const uploadPic = await userModel.findByIdAndUpdate(
          id,
